...
const chartPanelClassName =
  "flex min-h-0 flex-col overflow-hidden rounded-[1.28rem] border border-stone-200 bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,0.1),transparent_32%),linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(246,243,238,0.98)_100%)] p-4 shadow-[0_24px_40px_-36px_rgba(15,23,42,0.26)]";
    ...
    <header className="flex flex-col gap-2 xl:flex-row xl:items-start xl:justify-between">
      <div className="space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <h1 className="text-[1.42rem] font-semibold tracking-[-0.05em] text-slate-950 sm:text-[1.58rem]">
            매출 흐름과 후속 관리 숫자를 한 화면에서 확인합니다.
          </h1>
          <TonePill
      ...
      {metrics.map((metric) => {
        ...
        return (
          <article
            key={metric.label}
            className={joinClassNames(
              "relative flex h-full min-h-[5.7rem] flex-col justify-between overflow-hidden rounded-[1.15rem] border px-4 py-2.5 shadow-[0_18px_34px_-34px_rgba(15,23,42,0.22)]",
              accent.frame,
            )}
          >
            <div
            ...
            <div className="space-y-1">
              ...
              <p className="text-[1.5rem] font-semibold leading-none tracking-[-0.06em] text-slate-950 [font-variant-numeric:tabular-nums]">
                {metric.value}
              </p>
            </div>
...
function OperationSpotlightCard({
  ...
  return (
    <article
      className={joinClassNames(
        chartPanelClassName,
        "h-[20rem] space-y-3 text-slate-900",
        toneClassName,
      )}
    >
      <div className="flex items-start justify-between gap-3">
...
function OperationsOverview({ report }: { report: DashboardReportData }) {
...
export function DashboardOverview({ report }: DashboardOverviewProps) {
  return (
    <div className="space-y-3.5 p-4 sm:p-5 lg:p-6">
      ...
      <OperationsOverview report={report} />
    </div>
  );
}
