...
- **Global scope**: `C:/Project/PhoneShop/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Project/PhoneShop/.codex/notes/`)
...
2. Glob `C:/Project/PhoneShop/.codex/notes/*.md` (if directory exists) — global notes
...
Global (C:/Project/PhoneShop/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Project/PhoneShop/.codex/notes/` — works in any directory
